...
* 安全转换命令（强制编码转换）
  ...
  ```shell
  ffmpeg -i 星际争霸2.人族战役.游戏CG.SC_03.avi -c:v libx264 -profile:v high -pix_fmt yuv420p -c:a aac -b:a 192k -movflags +faststart output.mp4
  ```
  
  参数作用：  
  - -pix_fmt yuv420p：确保兼容所有播放器
  
  - -profile:v high：启用H.264高级特性
